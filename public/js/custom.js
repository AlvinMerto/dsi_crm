...
$(document).on('click', 'a[data-ajax-over1="true"], button[data-ajax-over1="true"], div[data-ajax-over1="true"]', function () {

    var title = $(this).data('title');
    var size = ($(this).data('size') == '') ? 'md' : $(this).data('size');
    var url = $(this).data('url');
    $("#commonModalOver1 .modal-title").html(title);
    $("#commonModalOver1 .modal-dialog").addClass('modal-' + size);
    $.ajax({
        url: url,
        success: function (data) {
            $('#commonModalOver1 .modal-body').html(data);
            $("#commonModalOver1").modal('show');
            commonLoader();
        },
        error: function (data) {
...
function common_bind(selector = "body") {
    ...
    if ($(".curdatepicker-input").length) {
        const d_disable = new Datepicker(document.querySelector('.curdatepicker-input'), {
            buttonClass: 'btn',
            autohide: true,
