...
class SalesAccountController extends Controller
{
    ...
    public function store(Request $request)
    {
        if(\Auth::user()->can('salesaccount create'))
        {
            ...
            $salesaccount['updated_by']          = creatorId();
            $salesaccount->save();
            Stream::create(
                [
    ...
    public function edit(SalesAccount $salesaccount)
    {
        if(\Auth::user()->can('salesaccount edit'))
        {
            ...
            $supporttickets  = Ticket::where('account_id', $salesaccount->id)->where('workspace_id',getActiveWorkSpace())->get();
            $notes=Notes::where('account_id', $salesaccount->id)->where('workspace_id',getActiveWorkSpace())->get();
            $projects=Project::where('account_id', $salesaccount->id)->where('workspace',getActiveWorkSpace())->get();
            $allactivities=AllActivityLog::where('account_id', $salesaccount->id)->where('workspace',getActiveWorkSpace())->get();
            ...
            return view('sales::salesaccount.edit', compact('meetings','salesorders','calls','salesinvoices','quotes','salesaccount', 'accountype', 'industry', 'user','previous', 'next', 'contacts', 'opportunitiess','cases','documents', 'streams', 'document_id','supporttickets','notes','projects','allactivities'));
        }
        else
        {
            return redirect()->back()->with('error', 'permission Denied');
    ...
    public function update(Request $request,SalesAccount $salesaccount)
    {
        if(\Auth::user()->can('salesaccount edit'))
        {
            ...
            $salesaccount['description']         = $request->description;
            $salesaccount['workspace']           = getActiveWorkSpace();
            $salesaccount['updated_by']          = \Auth::user()->id;
            $salesaccount->update();
