...
class ContactController extends Controller
{
   /**
    ...
    public function index()
    {
        ...
        {
            $contacts = Contact::where('created_by', creatorId())->where('workspace',getActiveWorkSpace())->orderBy('id','ASC')->get();
...
//          $contact['user_id']            = $request->user_id;
    ...
    public function edit(Contact $contact)
    {
        if(\Auth::user()->can('contact edit'))
        {
            ...
            $salesaccounts = SalesAccount::where('id',$contact->account)->where('created_by', creatorId())->where('workspace',getActiveWorkSpace())->get();
            $supporttickets  = Ticket::where('contact_id', $contact->id)->where('workspace_id',getActiveWorkSpace())->get();
            $notes = Notes::where('contact_id', $contact->id)->where('workspace_id',getActiveWorkSpace())->get();
            $projects=Project::where('contact_id', $contact->id)->where('workspace',getActiveWorkSpace())->get();
            $allactivities=AllActivityLog::where('contact_id', $contact->id)->where('workspace',getActiveWorkSpace())->get();
            ...
            return view('sales::contact.edit', compact('contact','quotes','salesorders','salesinvoices','cases','calls','meetings','opportunitiess','account','streams','user', 'previous', 'next','salesaccounts','supporttickets','notes','projects','allactivities'));
        }
        else
        {
            return redirect()->back()->with('error', 'permission Denied');
    ...
    {
        $account=SalesAccount::find($request->account_id);
        ...
        $msg=[
            ...
            'postalcode' => $account->billing_postalcode,
