...
<div>
    ...
    <a href="{{ route('salesinvoice.edit', $invoice->id) }}"
        class="btn btn-sm btn-info btn-icon" data-bs-toggle="tooltip"
        data-title="{{ __('invoice Edit') }}" title="{{ __('Edit') }}"><i class="ti ti-pencil"></i>
    </a>
    @endcan
                    ...
                    <div class="card-body">
                        <div class="row align-items-center mb-4">
                            <div class="col-sm-6 mb-3 mb-sm-0">
                                ...
                                <span class="col-sm-8"><span class="text-sm">{{ Modules\Sales\Entities\SalesInvoice::invoiceNumberFormat($invoice->invoice_id) }}</span></span>
                            </div>
                            <div class="col-lg-6 col-md-8 mt-3">
                                <h6 class="d-inline-block m-0 d-print-none">{{__('Invoice Date')}}</h6>
                                <span class="col-sm-8"><span class="text-sm">{{company_date_formate($invoice->created_at)}}</span></span>
                            </div>
                                <h6 class="d-inline-block m-0 d-print-none mt-3">{{__('Invoice')}}</h6>
                                @if($invoice->status == 0)
                                    <span class="badge bg-secondary p-2 px-3 rounded">{{ __(Modules\Sales\Entities\SalesInvoice::$status[$invoice->status]) }}</span>
                                @elseif($invoice->status == 1)
                                    <span class="badge bg-danger p-2 px-3 rounded">{{ __(Modules\Sales\Entities\SalesInvoice::$status[$invoice->status]) }}</span>
                            ...
                            <div class="col-sm-6 text-sm-end">
                                <div>
                                    ...
                                    <div class="float-end mt-3">
                                        {!! DNS2D::getBarcodeHTML(route('pay.salesinvoice',\Illuminate\Support\Facades\Crypt::encrypt($invoice->id)), "QRCODE",2,2) !!}
                                    </div>
                                ...
                                <div class="table-responsive mt-4">
                                    <table class="table invoice-detail-table">
                                        ...
                                        @foreach ($invoice->items as $key=>$invoiceitem)
                                            ...
                                            @endphp
                                            <tr>
                                                <td>{{(!empty($invoiceitem->items())?$invoiceitem->items()->name:'')}} </td>
                                    ...
                                    <table class="table invoice-table ">
                                        ...
                                        @endif
                                            <tr>
                                                ...
                                                <td>
                                                    <hr />
                                                    <h5 class="text-primary subTotal">  {{ currency_format_with_sym($invoice->getTotal()) }}</h5>
                                                </td>
                                            </tr>
                                        </tbody>
                                    </table>
                ...
                <ul class="list-group list-group-flush">
                    <li class="list-group-item px-0">
                        <div class="row align-items-center">
                            ...
                            <dd class="col-sm-12"><span class="text-sm">{{company_date_formate($invoice->created_at)}}</span></dd>
                        </div>
                    </li>
                </ul>
            </div>
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        for (var i = 0; i < invoiceItems.taxes.length; i++) {
                            taxes += '<span class="badge bg-primary p-2 mx-1 rounded">' + invoiceItems.taxes[i]
