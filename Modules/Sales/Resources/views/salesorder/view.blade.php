...
<div>
    ...
    <a href="#" class="btn btn-sm btn-warning btn-icon cp_link" data-link="{{route('pay.salesorder',\Illuminate\Support\Facades\Crypt::encrypt($salesOrder->id))}}" data-bs-toggle="tooltip" data-title="{{__('Click to copy SalesOrder link')}}"title="{{__('copy link')}}"><span class="btn-inner--icon text-white"><i class="ti ti-file"></i></span></a>
    @can('salesorder edit')
        <a href="{{ route('salesorder.edit',$salesOrder->id) }}" class="btn btn-sm btn-info btn-icon" data-bs-toggle="tooltip" data-title="{{__('Sales order Edit')}}" title="{{__('Edit')}}"><i class="ti ti-pencil"></i>
            </a>
    @endcan
    @if(module_is_active('ProductService'))
        <a href="#" data-size="md" data-url="{{ route('salesorder.salesorderitem',$salesOrder->id) }}" data-ajax-popup="true" data-bs-toggle="tooltip" data-title="{{__('Create New SalesOrder')}}" title="{{__('Create')}}" class="btn btn-sm btn-primary btn-icon">
                    ...
                    <div class="card-body">
                        <div class="row align-items-center mb-4">
                            <div class="col-sm-6 mb-3 mb-sm-0">
                                ...
                                <span class="col-sm-8"><span class="text-sm">{{ Modules\Sales\Entities\SalesOrder::salesorderNumberFormat($salesOrder->salesorder_id) }}</span></span>
                            </div>
                            <div class="col-lg-6 col-md-8 mt-3">
                                <h6 class="d-inline-block m-0 d-print-none">{{__('SalesOrder Date')}}</h6>
                                <span class="col-sm-8"><span class="text-sm">{{company_date_formate($salesOrder->created_at)}}</span></span>
                            </div>
                                    ...
                                    <table class="table invoice-detail-table">
                                        ...
                                        @foreach($salesOrder->items as $key => $salesOrderitem)
                                            ...
                                            <tr>
                                                        ...
                                                        @php $totalTaxRate = 0;@endphp
                                                            @foreach($salesOrderitem->tax($salesOrderitem->tax) as $tax)
                                                                @php
                                                                    ...
                                                                    $data+=$taxPrice;
                                                                @endphp
                                                                <a href="#!" class="d-block text-sm text-muted">{{$tax->name .' ('.$tax->rate .'%)'}} &nbsp;&nbsp;{{currency_format_with_sym($taxPrice)}}</a>
                                                            @endforeach
                                                            @php
                                                ...
                                                <td style="white-space: break-spaces;">{{!empty($salesOrderitem->description)?$salesOrderitem->description:'--'}} </td>
                                                @php
                                                    $tr_tex = (array_key_exists($key,$TaxPrice_array) == true) ? $TaxPrice_array[$key] : 0;
                                                @endphp
                                                <td class="text-right"> {{currency_format_with_sym(($salesOrderitem->price*$salesOrderitem->quantity)-$salesOrderitem->discount+$tr_tex)}}</td>
                                                <td class="text-right">
                                                    @if(module_is_active('ProductService'))
                                                    @can('salesorder edit')
                                                    <div class="action-btn bg-info ms-2">
                                                        <a href="#" data-url="{{ route('salesorder.item.edit',$salesOrderitem->id) }}" data-ajax-popup="true" class="mx-3 btn btn-sm d-inline-flex align-items-center text-white" data-bs-toggle="tooltip"  data-title="{{__('Edit Item')}}" data-original-title="{{__('Edit')}}" title="{{__('Edit Item')}}"><i class="ti ti-pencil"></i></a>
                                                    </div>
                                                    @endcan
                                                    @endif
                                ...
                                <div class="invoice-total">
                                    <table class="table invoice-table ">
                                        <tbody>
                                            ...
                                            <tr>
                                                <th>{{__('Discount')}} :</th>
                                                <td>{{currency_format_with_sym($salesOrder->getTotalDiscount())}}</td>
                                            </tr>
                                            @if(!empty($taxesData))
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <hr />
                                                    <h5 class="text-primary subTotal"> {{currency_format_with_sym($salesOrder->getTotal())}}</h5>
                                                </td>
                                            </tr>
                                        </tbody>
                                    </table>
                ...
                <ul class="list-group list-group-flush">
                    <li class="list-group-item px-0">
                        <div class="row align-items-center">
                            ...
                            <dd class="col-sm-12"><span class="text-sm">{{company_date_formate($salesOrder->created_at)}}</span></dd>
                        </div>
                    </li>
                </ul>
            </div>
