...
<div>
    ...
    @if(Auth::user()->type == 'company')
            <a class="btn btn-sm btn-warning btn-icon cp_link" data-link="{{route('pay.quote',\Illuminate\Support\Facades\Crypt::encrypt($quote->id))}}" data-bs-toggle="tooltip" data-title="{{__('Click to copy Quote link')}}"title="{{__('copy link')}}"><span class="btn-inner--icon text-white"><i class="ti ti-file"></i></span></a>
    @endif
    @can('quote edit')
            <a href="{{ route('quote.edit',$quote->id) }}" class="btn btn-sm btn-info btn-icon" data-bs-toggle="tooltip" title="{{__('Edit')}}" data-title="{{__('Edit Quote')}}"><i class="ti ti-pencil"></i></a>
        @endcan
    @if(module_is_active('ProductService'))
        <a data-url="{{ route('quote.quoteitem',$quote->id) }}" data-size="md" data-ajax-popup="true" data-bs-toggle="tooltip" data-title="{{__('Create New Quote Item')}}"title="{{__('Create')}}" class="btn btn-sm btn-primary btn-icon">
                        ...
                        <div class="row align-items-center mb-4">
                            <div class="col-sm-6 mb-3 mb-sm-0">
                                <div class="col-lg-6 col-md-8 pl-0" >
                                    ...
                                    <span class="col-sm-8"><span class="text-sm">{{ Modules\Sales\Entities\Quote::quoteNumberFormat($quote->quote_id) }}</span></span>
                                </div>
                                <div class="col-lg-6 col-md-8 pl-0 mt-3">
                                    <h6 class="d-inline-block m-0 d-print-none">{{__('Quote Date')}}</h6>
                                    <span class="col-sm-8"><span class="text-sm">{{company_date_formate($quote->created_at)}}</span></span>
                                </div>
                                        ...
                                        <tbody>
                                            ...
                                            @foreach($quote->items as $key => $quoteitem)
                                                ...
                                                <tr>
                                                    ...
                                                    <td class="px-0">{{currency_format_with_sym($quoteitem->discount)}} </td>
                                                    <td>
                                                        <div class="col">
                                                            @php
                                                                $totalTaxPrice = 0;
                                                    ...
                                                    <td class="px-0"  style="white-space: break-spaces;">{{!empty($quoteitem->description)?$quoteitem->description:'--'}} </td>
                                                    @php
                                                        $tr_tex = (array_key_exists($key,$TaxPrice_array) == true) ? $TaxPrice_array[$key] : 0;
                                                    @endphp
                                                    <td class="text-right">{{currency_format_with_sym(($quoteitem->price * $quoteitem->quantity)-$quoteitem->discount+$tr_tex)}}</td>
                                                    <td>
                                                        @if(module_is_active('ProductService'))
                                                        @can('quote edit')
                                                        <div class="action-btn bg-info ms-2">
                                                            <a data-url="{{ route('quote.quoteitem.edit',$quoteitem->id) }}" data-ajax-popup="true" class="mx-3 btn btn-sm d-inline-flex align-items-center text-white" data-bs-toggle="tooltip" title="{{__('Edit')}}" data-title="{{__('Edit Quote')}}"><i class="ti ti-pencil"></i></a>
                                                        </div>
                                                        @endif
                                                        @endcan
                                                        @can('quote delete')
                                    ...
                                    <table class="table invoice-table ">
                                        <tbody>
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <hr />
                                                    <h5 class="text-primary subTotal">  {{currency_format_with_sym($quote->getTotal())}}</h5>
                                                </td>
                                            </tr>
                                        </tbody>
                                    </table>
