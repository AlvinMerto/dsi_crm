...
class CreateOpportunitiesLis
{
    ...
    public function handle($event)
    {
        if (module_is_active('ActivityLog')) {
            ...
            if(isset($user->name))
            {
                $activity['description']    = __('New Opportunity Created for ') . $user->name . __(' by the ') . Auth::user()->name . '.';
            }
            else
            {
                $activity['description']    = __('New Opportunity Created by the ') . Auth::user()->name . '.';
            }
            $activity['url']            = '';
