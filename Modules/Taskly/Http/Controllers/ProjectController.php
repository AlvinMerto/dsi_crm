    ...
    */
    ...
    public function create($type,$id)
    {
        if(Auth::user()->can('project create'))
        {
            ...
            return view('taskly::projects.create',compact('customFields','workspace_users','accounts','type','id','all_contacts'));
        }
        else
        {
    ...
    public function edit(Project $project)
    {
        if(Auth::user()->can('project edit'))
        {
            if(module_is_active('CustomField')){
                ...
                $customFields             = \Modules\CustomField\Entities\CustomField::where('workspace_id', '=', getActiveWorkSpace())->where('module', '=', 'taskly')->where('sub_module','projects')->get();
                $accounts      = SalesAccount::where('workspace', getActiveWorkSpace())->get()->pluck('name', 'id');
                $contacts=Contact::where('account',$project->account_id)->get()->pluck('name','id');
            }else{
                $customFields = null;
            }
            return view('taskly::projects.edit',compact('project','customFields','accounts','contacts'));
        }
        else
        {
            return response()->json(['error' => __('Permission denied.')], 401);
